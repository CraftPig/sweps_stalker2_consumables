if CLIENT then 
    SWEP.WepSelectIcon = surface.GetTextureID( "vgui/hud/vgui_stalker2_bandage" )
...
SWEP.ViewModel = "models/weapons/sweps/stalker2/bandage/v_item_bandage.mdl"
SWEP.WorldModel = "models/weapons/sweps/stalker2/bandage/w_item_bandage.mdl"
SWEP.UseHands = true
...
local ID_WEAPON = "weapon_stalker2_bandage"
local ID_PRIMARYAMMO = "bandage"
...
function SWEP:InitializeConsumable()
	...
	timer.Simple(SequenceDuration * 0.5, function() -- Call item effects.
    ...
    timer.Simple(SequenceDuration, function() -- End of logic, strip weapon.
        if IsValid(self) and owner:Alive() then
			...
			if owner:GetAmmoCount(ID_PRIMARYAMMO) == 0 then 	
				owner:StripWeapon(ID_WEAPON)
			elseif owner:GetAmmoCount(ID_PRIMARYAMMO) ~= 0 and file.Exists("lua/AIS/AIS_Items.lua","GAME") then
				owner:AddAISItem("STALKER2Bandage",false)
				owner:StripWeapon(ID_WEAPON) 
			end
...
function SWEP:Heal(owner)
	if IsValid(self) and owner:GetActiveWeapon():GetClass() == ID_WEAPON then
		...
		if file.Exists("lua/SEF/SEF_Functions.lua","GAME") and SERVER then
			owner:ApplyEffect("Healing", 3, 1, 0.09)
