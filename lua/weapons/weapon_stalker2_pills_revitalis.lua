if CLIENT then 
    SWEP.WepSelectIcon = surface.GetTextureID( "vgui/hud/vgui_stalker2_revitalis" )
	SWEP.BounceWeaponIcon = true 
    SWEP.DrawWeaponInfoBox = true
end

SWEP.PrintName = "Revitalis Initializers"
...
SWEP.ViewModel = "models/weapons/sweps/stalker2/pills/v_item_pills.mdl"
SWEP.WorldModel = "models/weapons/sweps/stalker2/pills/w_item_pills.mdl"
SWEP.UseHands = true
...
local ID_WEAPON = "weapon_stalker2_pills_revitalis"
local ID_PRIMARYAMMO = "regenerator"
...
function SWEP:InitializeConsumable()
    ...
    timer.Simple(SequenceDuration * 0.9, function() -- Call item effects.
    ...
    timer.Simple(SequenceDuration, function() -- End of logic, strip weapon.
        if IsValid(self) and owner:Alive() then
			...
			elseif owner:GetAmmoCount(self.Primary.Ammo) ~= 0 and file.Exists("lua/AIS/AIS_Items.lua","GAME") then
				owner:AddAISItem("STALKER2Revitalis",false)
				owner:StripWeapon(ID_WEAPON) 
			end
...
function SWEP:Heal(owner)
	if IsValid(self) and owner:GetActiveWeapon():GetClass() == ID_WEAPON then
		...
		if file.Exists("lua/SEF/SEF_Functions.lua","GAME") and SERVER then
			owner:ApplyEffect("Revitalis", 120, 1, 1)
		end
...
function SWEP:Think()
	local owner = self:GetOwner() 
	owner:ResetSequenceInfo()
	owner:SetSequence("menu_gman")
end
